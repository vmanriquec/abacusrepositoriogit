...
' This is a partial class file. The other one is idcrdEntity.cs
...
Namespace Entities
    ...
    Public Partial Class idcrd
        Inherits Objects.idcrdObject
        Implements IMappeableidcrd
        Implements IEquatable(Of idcrd)
...
End If
        ...
        ''' </summary>
        Public Shadows Function OriginalValue() As idcrd 
                Return CType(MyBase.OriginalValue, idcrd)
        End Function
        ...
        Protected Overrides Function Clone() As Object 
            Dim newObject As idcrd 
            

            newObject = CType(Me.MemberwiseClone(), idcrd)
            ' Entities
            ...
            ' OriginalValue
            Dim newOriginalValue As idcrd
            If MyBase.InternalOriginalValue IsNot Nothing Then
                newOriginalValue = CType(Me.OriginalValue().MemberwiseClone(), idcrd)
                ' Entities
        ...
        Private Sub CompleteEntity(ClientesEntity As Objects.ClientesObject, rutaEntity As Objects.rutaObject) Implements IMappeableidcrd.CompleteEntity
        _ClientesEntity = ClientesEntity
_rutaEntity = rutaEntity
        End Sub

        
        Private Function IsClientesEntityNull() As Boolean Implements IMappeableidcrd.IsClientesEntityNull
            Return IsNothing(_ClientesEntity)
        End Function
        
        Private Function IsrutaEntityNull() As Boolean Implements IMappeableidcrd.IsrutaEntityNull
            Return IsNothing(_rutaEntity)
        End Function
        

        Private Sub SetFKValuesForChilds(entity As idcrd) Implements IMappeableidcrd.SetFKValuesForChilds
                
        End Sub

        Public Overloads Function Equals(other As idcrd) As Boolean Implements IEquatable(Of idcrd).Equals
            Return UniqueIdentifierHelper.IsSameObject(CType(Me, IUniqueIdentifiable), CType(other, IUniqueIdentifiable))
        End Function

    End Class

    Public Interface IMappeableidcrd
        ...
        Sub SetFKValuesForChilds(entity As idcrd )
    End Interface

    Public Partial Class idcrdList
        Inherits ObjectList(Of idcrd)    
    End Class
End Namespace

Namespace Views
    Partial Public Class idcrdListView
        Inherits ObjectListView(Of Entities.idcrd)

        Sub New(ByVal list As Entities.idcrdList)
            MyBase.New(list)
        End Sub
    End Class
End NameSpace
